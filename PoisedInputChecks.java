...
public class PoisedInputChecks {  // Main class declaration.
		...
		public static String stringCheck(String type) {
			 // While loop repeatedly re-prompts for input until correct.
			while(true) {  
				Scanner userInput = new Scanner(System.in);
				String input = userInput.nextLine();
				 // Checking if the input is empty, too short, or too long.
				if ((input == null) || (input.length() > 150)) {  
					System.out.println("Incorrect entry. Please re-enter the " + type + ": \n");
					
				} else {
					// Returning the user's correctly inputed string.
					return input;  
		...
		/**
		 * This method verifies user input when asked to enter a float number (i.e. number with decimal point).
		 * @param type type describes the input required for the user to enter
